...
function extractProductData() {
  try {
    ...
    if (!title || title.toLowerCase().includes('buy') || title.toLowerCase().includes('meesho')) {
    ...
    function isPromotionalImage(img, allowSmall = false) {
      ...
      if (src.includes('marketing') || src.includes('banner') || src.includes('promo')) {
        console.log('Filtered marketing image:', src);
        return true;
      }
      
      // Filter by alt text
      if (alt.includes('offer') || alt.includes('discount') || alt.includes('sale') || alt.includes('%')) {
      ...
      if (parent) {
        const bgStyle = parent.style.background || parent.style.backgroundColor || '';
        if (bgStyle.includes('rgb(0') || bgStyle.includes('#0') || bgStyle.includes('black')) {
          console.log('Filtered image with dark background (likely promotional)');
          return true;
        }
      }
    ...
    allImgs.forEach(img => {
      if (img.src && img.src.includes('images.meesho') && img.src.includes('/products/')) {
        ...
        if (!isPromotionalImage(img, true)) {
          ...
          console.log('Found product image candidate:', highResUrl, `Size: ${img.width}x${img.height}, Area: ${area}`);
        }
      }
    });
    ...
    console.log(`Found ${sortedImages.length} valid product images after filtering`);
    ...
    sortedImages.forEach((imgData, index) => {
      if (images.length < 6 && !seenUrls.has(imgData.url)) {
        ...
        console.log(`Added product image ${index + 1}:`, imgData.url, `(${imgData.width}x${imgData.height})`);
      }
    });
    ...
    if (images.length < 3) {
      allImgs.forEach(img => {
        if (img.src && img.src.includes('images.meesho')) {
          if (!isPromotionalImage(img) && img.width > 150 && img.height > 150 && img.width <= 800) {
    ...
    if (allText.includes('Product Highlights')) {
      ...
      if (highlightsIdx > -1 && additionalIdx > highlightsIdx) {
        ...
        const highlightsText = allText.substring(highlightsIdx + 18, additionalIdx).trim();
    ...
    if (allText.includes('Additional Details')) {
      ...
      const detailsText = allText.substring(detailsIdx + 18, detailsIdx + 800).trim();
