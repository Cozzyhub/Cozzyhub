...
describe('Affiliate Product Link API', () => {
  ...
  test('stores product-specific commission rate correctly', async () => {
    ...
    await supabase.from('affiliates').select('*').eq('user_id', 'user-123').eq('status', 'active').single();
    await supabase.from('products').select('*').eq('id', 'prod-456').single();
    await supabase.from('product_affiliate_links').select('*').eq('affiliate_id', 'aff-123').eq('product_id', 'prod-456').eq('is_active', true).single();
    await supabase.rpc('generate_product_link_code', { aff_id: 'aff-123', prod_id: 'prod-456' });
