...
const createMockHeaders = () => {
  ...
  return {
    ...
    _setHeader: (key: string, value: string) => headersMap.set(key.toLowerCase(), value),
  };
};
...
describe('Affiliate Tracking API', () => {
  ...
  test('accurately records clicks for general referral codes', async () => {
    ...
    // Set up mock headers
    headers._setHeader('user-agent', 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) Chrome/91.0');
  ...
  test('accurately records clicks for product-specific link codes', async () => {
    ...
    headers._setHeader('user-agent', 'Mozilla/5.0 (iPhone; CPU iPhone OS 14_0)');
  ...
  test('extracts IP address from x-forwarded-for header (first IP)', async () => {
    const headers = createMockHeaders();
    headers._setHeader('x-forwarded-for', '192.168.1.100, 10.0.0.1, 172.16.0.1');
